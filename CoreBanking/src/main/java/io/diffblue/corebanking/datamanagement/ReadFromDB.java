...
public class ReadFromDB {
  ...
  private static void tempStaticData(CoreBanking coreBanking) throws TransactionException {
    ...
    Account acc1cli1 = coreBanking.openNewAccount(client1, 100, new Branch());
    Account acc2cli1 = coreBanking.openNewAccount(client1, 100, new Branch());
    Account acc3cli1 = coreBanking.openNewAccount(client1, 100, new Branch());

    // Client 2
    Client client2 = new Client("Jane Robbins");

    Account acc1cli2 = coreBanking.openNewAccount(client2, 100, new Branch());
    Account acc2cli2 = coreBanking.openNewAccount(client2, 100, new Branch());

    // Client 3
    Client client3 = new Client("Emily Simmons");

    Account acc1cli3 = coreBanking.openNewAccount(client3, 100, new Branch());
